...
```bash
docker build -f .devcontainer/Dockerfile -t cdsl-dev:test .
```
...
```bash
docker run --rm cdsl-dev:test bash -c "bash .devcontainer/post-create.sh"
```
...
```bash
docker run --rm -it cdsl-dev:test bash
```
...
- ✅ Build Maven project (if pom.xml exists)
- ✅ Create helper scripts (dev_helpers.sh, test_environment.sh)
